...
import bracu.shout.out.v1.pkg1.login;
...
public class DBConnect extends login
{
    ...
    public boolean login(String u, String p) throws SQLException
    {
        try
        {
            String query = "select pass from authentication where user='"+u+"';";
    ...
    public void sCreate(String n, String i, String d, String u, String p) throws SQLException
    {
        try
        {
            String query = "insert into authentication values ('"+u+"', '"+p+"');";
            st.executeUpdate(query);
            query = "insert into stdProfile values ('"+n+"', '"+i+"', '"+d+"' , '"+u+"', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A');";
            st.executeUpdate(query);
        }
        catch(SQLException ex)
        {
    ...
    public void fCreate(String n, String i, String d, String u, String p) throws SQLException
    {
        try
        {
            String query = "insert into authentication values ('"+u+"', '"+p+"');";
            st.executeUpdate(query);
            query = "insert into facProfile values ('"+n+"', '"+i+"', '"+d+"' , '"+u+"', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A');";
            st.executeUpdate(query);
        }
        catch(SQLException ex)
        {
    ...
    public boolean checkUser(String u)
    {
        try
        {
            String query = "select user from authentication;";
            rs= st.executeQuery(query);
            while(rs.next())
            {
    ...
    {
        String str=null;
        String query = "select name from stdprofile where user='"+u+"';";
        rs= st.executeQuery(query);
        while(rs.next())
        {
        ...
        if(str==null)
        {
            query = "select name from facprofile where user='"+u+"';";
            rs= st.executeQuery(query);
            while(rs.next())
            {
                str=rs.getString("name");
    ...
    {
        String str=null;
        String query = "select id from stdprofile where user='"+u+"';";
        rs= st.executeQuery(query);
        while(rs.next())
        {
            str= rs.getString("id");
        ...
        if(str==null)
        {
            query = "select id from facprofile where user='"+u+"';";
            rs= st.executeQuery(query);
            while(rs.next())
            {
                str= rs.getString("id");
    ...
    {
        String str=null;
        String query = "select dept from stdprofile where user='"+u+"';";
        rs= st.executeQuery(query);
        while(rs.next())
        {
        ...
        if(str==null)
        {
            query = "select dept from facprofile where user='"+u+"';";
            rs= st.executeQuery(query);
            while(rs.next())
            {
                str= rs.getString("dept");
    ...
    {
        String str=null;
        String query = "select email from stdprofile where user='"+u+"';";
        rs= st.executeQuery(query);
        while(rs.next())
        {
        ...
        if(str==null)
        {
            query = "select email from facprofile where user='"+u+"';";
            rs= st.executeQuery(query);
            while(rs.next())
            {
                str= rs.getString("email");
    ...
    {
        String str=null;
        String query = "select blood from stdprofile where user='"+u+"';";
        rs= st.executeQuery(query);
        while(rs.next())
        {
        ...
        if(str==null)
        {
            query = "select blood from facprofile where user='"+u+"';";
            rs= st.executeQuery(query);
            while(rs.next())
            {
                str= rs.getString("blood");
    ...
    {
        String res = null;
        String query = "select user, message from shout WHERE date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) order by date desc;"; 
        rs= st.executeQuery(query);
        String user="", message="";
        while(rs.next())
        {
    ...
    /*public boolean getData(String str)
    {
        try
        {
            String query = "select name, major, cgpa from lab_grade where student_id = "+str;
            rs= st.executeQuery(query);
            int disC=0;
            String name="",major="",cgpa="";
            while(rs.next())
            {
